    ...
    if (gSaveBlock1Ptr->tx_Challenges_Mirror && (gBattleTypeFlags & BATTLE_TYPE_TRAINER || gBattleTypeFlags & BATTLE_TYPE_DOUBLE))
    {
        if (!gSaveBlock1Ptr->tx_Challenges_Mirror_Thief)
        {
            for (j = 0; j < PARTY_SIZE; j++)
                //gPlayerPartyBackup[j] = gParties[B_TRAINER_PLAYER][j];
...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 halfTeam, u32 battleTypeFlags)
{
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (s32 i = 0; i < monsCount; i++)
        {
            ...
            {
                DebugPrintf("Trainers not randomized.");
                CreateMon(&party[i], partyData[monIndex].species, GetScaledLevel(partyData[i].lvl), personalityValue, otId);
            }
